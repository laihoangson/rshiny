#' Clean and process quarantine breaches data
#'
#' @return Cleaned breaches dataset
#' @export
clean_breaches_data <- function() {
  # Replicate breaches data from the Excel file
...
#' Clean cases summary data
#'
#' @return Cleaned cases summary dataset
#' @export
clean_cases_data <- function() {
  # Replicate cases summary data from the Excel file
  cases_data <- tibble::tribble(
    ~state, ~breaches, ~overseas_cases, ~ratio, ~report_date, ~period,
    "NSW", 10, 3479, "1 in 348", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "VIC", 4, 1192, "1 in 298", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "QLD", 8, 1355, "1 in 169", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "WA", 4, 916, "1 in 229", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "SA", 2, 616, "1 in 308", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "TAS", 0, 85, "-", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "ACT", 0, 95, "-", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021",
    "NT", 0, 173, "-", as.Date("2021-07-03"), "27 Oct 2020 to 3 July 2021"
  )

  return(cases_data)
}

#' Clean time series data for 7-day averages
#'
#' @return Cleaned time series dataset
#' @export
clean_timeseries_data <- function() {
  # Create sample time series data for states
  dates <- seq(as.Date("2020-10-27"), as.Date("2021-07-31"), by = "day")

  set.seed(123) # For reproducible random data
  timeseries_data <- data.frame(
    date = dates,
    nsw_overseas = round(runif(length(dates), 1, 10), 1),
    nsw_breach = sample(c(0, 0, 0, 0, 1), length(dates), replace = TRUE),
    vic_overseas = round(runif(length(dates), 0, 8), 1),
    vic_breach = sample(c(0, 0, 0, 0, 0, 1), length(dates), replace = TRUE),
    qld_overseas = round(runif(length(dates), 0, 6), 1),
    qld_breach = sample(c(0, 0, 0, 0, 1), length(dates), replace = TRUE)
  )
...
breaches_data <- clean_breaches_data()
cases_data <- clean_cases_data()
timeseries_data <- clean_timeseries_data()

# Save datasets to package
usethis::use_data(breaches_data, overwrite = TRUE)
usethis::use_data(cases_data, overwrite = TRUE)
usethis::use_data(timeseries_data, overwrite = TRUE)
